-- Migration to fix invalid rental transition on unlock bike
-- Ensures payment_required is false for app reservations, start_rental is idempotent,
-- and status = 'reserved' can transition to active directly without requiring backend authorization records.
...
begin
  ...
  -- Idempotency: if already started (e.g. unlock retried or network hiccup), return current rental
  if v_rental.status = 'active' then
    return v_rental;
  end if;

  if not (
    v_rental.status = 'reserved'
  ...
  if v_rental.status = 'authorized' and v_rental.payment_required and not exists (
    select 1
...
begin
  ...
  perform private.add_rental_event(v_rental.id, v_user_id, 'user_cancelled');
  return v_rental;
end;
$$;

-- Fix any current reserved rentals stuck with payment_required = true
update public.rentals
set payment_required = false
where status = 'reserved' and payment_required = true;
