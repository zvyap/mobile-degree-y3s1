-- Migration to improve bike status error messages during rental and deposit
alter table public.bikes drop constraint if exists bikes_status_check;
alter table public.bikes add constraint bikes_status_check
  check (status in ('available', 'reserved', 'in_use', 'maintenance', 'retired', 'lost', 'unavailable'));
...
begin
  ...
  perform private.add_rental_event(
    ...
    jsonb_build_object(
      'bike_id', v_bike.id,
      'start_station_id', v_bike.current_station_id,
      'rental_plan_id', v_plan.id
    )
  );
