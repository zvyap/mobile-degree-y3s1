...
create function private.reserve_rental_session(p_qr_token uuid)
...
begin
  ...
  if v_existing.id is not null then
    if v_existing.status = 'reserved'
      ...
      perform bike.id
      from public.bikes as bike
      where bike.id = v_existing.bike_id
      for update;
    ...
    elsif v_existing.bike_id = v_candidate_id then
      return v_existing;
    else
...
$$;
create function private.request_return_by_code(
  p_rental_id bigint,
  p_station_code text
)
returns public.rentals
...
begin
  ...
  where station.code = p_station_code and station.is_active;

  if v_station_id is null then
    raise exception using errcode = 'P0001', message = 'station_unavailable';
  end if;

  return private.request_return(p_rental_id, v_station_id);
end;
$$;
...
$$;
create function public.reserve_rental_session(p_qr_token uuid)
returns public.rentals
language sql
security invoker
set search_path = ''
...
$$;
create function public.request_return_by_code(
  p_rental_id bigint,
  p_station_code text
)
returns public.rentals
...
set search_path = ''
as $$
  select private.request_return_by_code(p_rental_id, p_station_code);
$$;
revoke execute on function private.reserve_rental_session(uuid)
from public, anon;
revoke execute on function private.request_return_by_code(bigint, text)
from public, anon;
grant execute on function private.reserve_rental_session(uuid)
to authenticated;
grant execute on function private.request_return_by_code(bigint, text)
to authenticated;
revoke execute on function public.reserve_rental_session(uuid)
from public, anon;
revoke execute on function public.request_return_by_code(bigint, text)
from public, anon;
grant execute on function public.reserve_rental_session(uuid)
to authenticated;
grant execute on function public.request_return_by_code(bigint, text)
to authenticated;
...
grant select (
  ...
  created_at,
  updated_at
) on public.bikes to authenticated;
